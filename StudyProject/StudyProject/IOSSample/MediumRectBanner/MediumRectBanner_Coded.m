...
#import <iAd/iAd.h>
#import "MediumRectBanner_Coded.h"

@interface ImageViewCellCode : UICollectionViewCell

@property (nonatomic, readonly) UIImageView *imageView;

@end

@implementation ImageViewCellCode
...
@interface BannerViewCellCode : UICollectionViewCell

@property (nonatomic, retain) ADBannerView *bannerView;

@end

@implementation BannerViewCellCode
...
@interface MediumRectBanner_Coded () <ADBannerViewDelegate>

@end

@implementation MediumRectBanner_Coded{
    ADBannerView *_banner;
    // We only want to insert/delete our banner if we are changing from Loaded to Non-loaded and vice versa, so we use this
    // ivar to track that state. If this wasn't a concern, we wouldn't need this ivar at all.
    BOOL _bannerWasLoaded;
}

- (instancetype)init{
    self = [super initWithNibName:@"MediumRectBanner_Coded" bundle:nil];
    return self;
}
...
- (void)viewDidLoad
{
    ...
    [self.collectionView registerClass:[ImageViewCellCode class] forCellWithReuseIdentifier:@"ImageView"];
    [self.collectionView registerClass:[BannerViewCellCode class] forCellWithReuseIdentifier:@"BannerView"];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_banner.bannerLoaded && [self isBannerItem:indexPath]) {
        BannerViewCellCode *bannerCell = [collectionView dequeueReusableCellWithReuseIdentifier:@"BannerView" forIndexPath:indexPath];
        bannerCell.bannerView = _banner;
        cell = bannerCell;
    } else {
        ImageViewCellCode *imageCell = [collectionView dequeueReusableCellWithReuseIdentifier:@"ImageView" forIndexPath:indexPath];
